...
-- رقم الهاتف يبقى محفوظاً لأغراض التواصل/واتساب فقط، وليس وسيلة دخول
-- Supabase / PostgreSQL
-- ============================================================
...
create table students (
  ...
  -- رقم الهاتف يُحفظ فقط لأغراض التواصل (واتساب) وليس وسيلة دخول
  phone text unique,
...
comment on column students.phone is 'رقم الهاتف لأغراض التواصل وواتساب فقط - ليس وسيلة دخول';
comment on column students.approval_status is 'pending = سجل بإيميله وينتظر تفعيل المعلم (يشوف رابط واتساب الحلقة فقط) | approved = مفعّل وله وصول كامل للوحته';
...
comment on column students.start_surah is 'نقطة بداية الطالب الأصلية (ثابتة، لا تتغير) - تُستخدم كحد المراجعة في الاتجاه forward';
comment on column students.review_direction is 'اتجاه الحفظ والمراجعة لهذا الطالب، قابل للتعديل اليدوي من المعلم';
comment on column students.daily_new_ayahs is 'تقدير عدد الآيات المتوقع حفظها بين كل حصتين تسميع، يُعدَّل يدوياً من المعلم';
...
comment on column reports.raw_range_text is 'الوصف الحر الكامل للمقرر كما يكتبه المعلم، مهم للحالات المركبة من عدة سور غير متصلة';
...
begin
  if extract(dow from new.review_date) = 6 then
    raise exception 'لا يُسجَّل تسجيل مراجعة ذاتية يوم السبت — هذا يوم المراجعة الشاملة والاختبار بالحلقة';
  end if;
  return new;
end;
$$ language plpgsql;
...
comment on view pending_students is 'الطلاب الذين سجلوا بإيميلهم وينتظرون أن يفعّلهم المعلم يدوياً';
